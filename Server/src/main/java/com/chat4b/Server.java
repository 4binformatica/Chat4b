...
import com.chat4b.Message.MessageData;
...
public class Server extends WebSocketServer {
    ...
    public Server(InetSocketAddress address, Config config) throws ClassNotFoundException, SQLException {
        super(address);
        mailClient = new MailClient(config.getMailHost(), config.getMailPort(), config.isMailAuth(),
                config.isMailStarttls(), config.getMailUser(), config.getMailPassword());
        this.config = config;
        checkDatabase();
     ...
     * @param handshake The handshake object contains the headers sent by the
     *                  client.
     */
     ...
     * @param code   The codes can be looked up
     *               [here](https://tools.ietf.org/html/rfc6455#section-7.4.1)
     * @param reason The reason for closing the connection. This must be provided as
     *               per the WebSocket
     *               protocol.
     * @param remote true if the closing of the connection was initiated by the
     *               remote host.
     */
     ...
     * </code>
     * 
     * @param message The message object that contains the message, sender, and
     *                receiver.
     */
    public void sendTo(Message message) throws SQLException {
        ...
        for (WebSocket conn : clients.get(message.getReceiver())) {
            if (conn != null) {
                ...
                conn.send(Message.toJson(message));
            }
        }
    }
    ...
    /**
     * If the database checkUser function returns true, return true. Otherwise,
     * return false
     * 
    ...
    /**
     * It sends a message to a user with the message "reload" and the sender
     * "Server"
     * 
    ...
    public boolean register(String username, String password, String mail) throws SQLException {
        return database.newUser(username, password,
                "https://i.ibb.co/NsJGFh6/istockphoto-522855255-612x612-modified.png", mail);
    }
    ...
    /**
     * It generates a random string of 10 characters based on the username, checks
     * if the loginID is
    ...
    /**
     * It takes a string, and returns a string of 10 random characters from the
     * original string
     * 
    ...
    /**
     * It takes a username and returns a string of 7 characters from the username,
     * and a random number
     * between 0 and 9
     * 
     ...
     * 
     * @param msg The message object that contains all the information about the
     *            message
     */
    public void manageOperation(Message msg) throws Exception {

        switch (msg.getOperation()) {
            // The above code is checking if the user is logged in. If the user is logged
            // in, the user
            // is added to the clients list. If the user is not logged in, the user is sent
            // a message
            ...
            case "login":
                if (login(msg.getUsername(), msg.getData())) {
                    ...
                    if (database.needVerification(msg.getUsername())) {
                        sendTo(msg.getConn(),
                                new Message("needVerification", msg.getUsername(), msg.getUsername(), "true"));
                    } else {
                        database.addLoginID(msg.getUsername(), generateLoginID(msg.getUsername()));
                        sendTo(msg.getConn(), new Message("loginID", msg.getUsername(), msg.getUsername(),
                                database.getLoginID(msg.getUsername())));
                        sendTo(msg.getConn(), new Message("login", msg.getUsername(), msg.getUsername(), "success"));
                    }
                } else {
                    sendTo(msg.getConn(), new Message("login", msg.getUsername(), msg.getUsername(),
                            "Login failed, wrong username or password"));
                }
                break;
            // The above code is checking if the verification code exists in the database.
            // If it does,
            // it will remove the verification code from the database and add a login ID to
            // the
            // database. It will then send the login ID to the client. If the verification
            // code does
            // not exist, it will send a message to the client saying that the verification
            // code is
            // false.
            case "checkVerificationCode":
                if (database.checkIfVerificationCodeExists(msg.getUsername(), msg.getData())) {
                    ...
                    sendTo(msg.getConn(), new Message("loginID", msg.getUsername(), msg.getUsername(),
                            database.getLoginID(msg.getUsername())));
                    sendTo(msg.getConn(),
                            new Message("checkVerificationCode", msg.getUsername(), msg.getUsername(), "success"));
                    database.removeVerificationCode(msg.getUsername(), msg.getData());
                } else {
                    sendTo(msg.getConn(),
                            new Message("checkVerificationCode", msg.getUsername(), msg.getUsername(), "false"));
                }
                break;
            // The above code is checking if the loginID is outdated and if it is not
            // generating a new
            // one.
            case "checkLoginID":
            ...
            case "send":
                ...
                break;
            // Checking if the username is already in use, if not it will generate a
            // verification code
            ...
            case "register":
                if (register(msg.getUsername(), msg.getData(), msg.getReceiver())) {
                    ...
                    mailClient.sendMail("no-reply@kapindustries.it", msg.getReceiver(), "Verification code",
                ...
                break;
            // Removing the login ID from the database and removing the client from the list
            // of
            // clients.
            case "logout":
                database.removeLoginID(msg.getUsername());
            ...
            case "message":
                ...
                System.out.println("Message received from " + msg.getUsername() + " to " + msg.getReceiver() + " "
                        + msg.getMessage());
                msg.setUserName(database.getUsernameByLoginID(msg.getUsername()));

                // maybe it's better to take the object using GSON but ill do it in the old way
                MessageData dat = msg.getMessageData();
                String base64 = dat.getImage();
                String url = uploadImage(base64).getUrl();
                dat.setImage(url);
                msg.setMessageData(dat);
            ...
            case "addContact":
                ...
                if (database.userExist(msg.getData())) {
                    ...
                    sendTo(msg.getConn(),
                            new Message("addContact", msg.getUsername(), msg.getData(), "User does not exist"));
                }
                break;
            ...
            case "getMessages":
                ...
                for (Message m : messages) {
                    if (m.getReceiver() == null) {
                        ...
                        if ((m.getReceiver().equals(msg.getUsername()) && m.getUsername().equals(msg.getData())
                                || (m.getReceiver().equals(msg.getData())
                                        && m.getUsername().equals(msg.getUsername())))) {
            ...
            case "getContacts":
                ...
                for (String contact : contacts) {
                    sendTo(msg.getConn(),
                            new Message("contact", contact, msg.getUsername(), database.getProfilePic(contact)));
                }
                break;
            // The above code is uploading an image to imgbb.com and then sending the image
            // to the
            // receiver.
            case "image":
                if (!database.validateLoginID(msg.getUsername())) {
                ...
                break;
            // The above code is uploading the image to imgbb.com and then changing the
            // profile picture
            // of the user.
            case "changeProfilePic":
                ...
                break;
            // The above code is getting the draft from the database and sending it to the
            // client.
            case "getDraft":
            ...
            case "saveDraft":
                ...
                database.createDraft(database.getUsernameByLoginID(msg.getUsername()), msg.getData(),
                        msg.getReceiver());
                break;
            ...
            case "changeBio":
                ...
                break;
            // Calculating the time difference between the time the message was sent and the
            // time the
            ...
            case "ping":
                ...
                break;
            // The above code is checking if the email exists in the database. If it does,
            // it will
            ...
            case "forgotPassword":
                if (database.checkMailExists(msg.getData())) {
                    ...
                    sendTo(msg.getConn(),
                            new Message("forgotPassword", msg.getUsername(), msg.getUsername(), "success"));
                    mailClient.sendMail("no-reply@kapindustries.it", msg.getData(), "Forgot password",
                            recoveryMail(forgotCode));
                } else {
                    sendTo(msg.getConn(),
                            new Message("forgetPassword", msg.getUsername(), msg.getUsername(), "Mail does not exist"));
                }
                break;
            ...
            case "checkForgotCode":
                if (database.checkForgotCode(msg.getData())) {
                    sendTo(msg.getConn(),
                            new Message("checkForgotCode", msg.getUsername(), msg.getUsername(), "success"));

                } else {
                    sendTo(msg.getConn(),
                            new Message("checkForgotCode", msg.getUsername(), msg.getUsername(), "Wrong code"));
                }
                break;
            ...
            case "changeForgotPassword":
                ...
                if (!database.checkIfCodeIsForMail(mail, code)) {
                    sendTo(msg.getConn(),
                            new Message("changeForgotPassword", msg.getUsername(), msg.getUsername(), "Wrong code"));
                    return;
                }
                database.updatePasswordWithMail(mail, msg.getData());
                database.removeForgotCode(msg.getUsername());
                sendTo(msg.getConn(),
                        new Message("changeForgotPassword", msg.getUsername(), msg.getUsername(), "success"));
                break;
            // Checking if the mail exists in the database. If it does, it sends a message
            // to the
            // client saying that the mail already exists. If it doesn't, it changes the
            // mail in the
            // database and sends a message to the client saying that the change was
            // successful.
            case "changeMail":
                if (database.checkMailExists(msg.getData())) {
                    sendTo(msg.getConn(),
                            new Message("changeMail", msg.getUsername(), msg.getUsername(), "Mail already exists"));
                } else {
                ...
                break;
            // Checking if the user is an administrator. If the user is an administrator, it
            // will
            ...
            case "isAdministrator":
                if (database.isAdministator(msg.getUsername())) {
                    ...
                    mailClient.sendMail("no-reply@kapindustries.it", database.getMail(msg.getUsername()), "Admin Code",
                            "Hi " + msg.getUsername() + ", your admin code is " + adminCode
                                    + ". Please do not share it with anyone."
                                    + "\n If you did not request this code, contact Administrator immediately. \n This code will expire in 5 minutes.");
                } else {
                    sendTo(msg.getConn(),
                            new Message("isAdministrator", msg.getUsername(), msg.getUsername(), "false"));
                }
                break;
            ...
            case "verifyAdminCode":
                if (database.verifyAdminCode(msg.getUsername(), msg.getData())) {
                    ...
                    sendTo(msg.getConn(),
                            new Message("verifyAdminCode", msg.getUsername(), msg.getUsername(), "false"));
                }
                break;
            // Getting all the users from the database and sending them to the client.
            case "getAllUsers":
                if (!database.isAdministator(msg.getUsername())
                        || !database.verifyAdminCode(msg.getUsername(), msg.getReceiver())) {
                ...
                for (String user : users) {
                    sendTo(msg.getConn(),
                            new Message("getAllUsers", user, database.getMail(user), database.getProfilePic(user)));
                }
                break;
            ...
            case "deleteUser":
                ...
                break;
            // Checking if the username exists in the database. If it does, it sends a
            // message to the
            // client saying that the username already exists. If it doesn't, it changes the
            // username
            // in the database and sends a message to the client saying that the username
            // was changed
            // successfully.
            case "changeUsername":
                if (database.checkUsernameExists(msg.getData())) {
                    sendTo(msg.getConn(), new Message("changeUsername", msg.getUsername(), msg.getUsername(),
                            "Username already exists"));
                } else {
                    database.changeUsername(msg.getUsername(), msg.getData());
                    sendTo(msg.getConn(),
                            new Message("changeUsername", msg.getUsername(), msg.getUsername(), "success"));
                }
                break;
            // Checking if the email exists in the database. If it does, it sends a message
            // to the
            // client saying that the email already exists. If it doesn't, it changes the
            // email in the
            ...
            case "changeEmail":
                if (database.checkMailExists(msg.getData())) {
                    sendTo(msg.getConn(),
                            new Message("changeEmail", msg.getUsername(), msg.getUsername(), "Mail already exists"));
                } else {
                    database.changeMail(msg.getUsername(), msg.getData());
            ...
            case "deleteAllMessages":
                ...
                break;
            // A switch statement that is checking the value of the variable "action". If
            // the value of
            // "action" is "keepAlive", then the code will break out of the switch
            // statement.
            case "keepAlive":
                break;
            ...
            case "createGroup": {
                ...
                if (database.checkIfGroupExists(groupName)) {
                    sendTo(msg.getConn(),
                            new Message("createGroup", msg.getUsername(), msg.getUsername(), "Group already exists"));
                    return;
                }
                ...
                for (int i = 0; i < members.length; i++) {
                    if (!database.userExist(members[i])) {
                        sendTo(msg.getConn(), new Message("createGroup", msg.getUsername(), msg.getUsername(),
                                "User " + members[i] + " does not exist"));
                        continue;
                    }
            ...
            case "addUserToGroup": {
                ...
                if (!database.isAdminOfGroup(groupName, msg.getUsername())) {
                    sendTo(msg.getConn(), new Message("addUserToGroup", msg.getUsername(), msg.getUsername(),
            ...
            case "removeUserFromGroup": {
                ...
                if (!database.isAdminOfGroup(groupName, msg.getUsername())) {
                    sendTo(msg.getConn(), new Message("removeUserFromGroup", msg.getUsername(), msg.getUsername(),
                            "You are not admin of this group"));
                    return;
                }
                ...
                sendTo(msg.getConn(),
                        new Message("removeUserFromGroup", msg.getUsername(), msg.getUsername(), "success"));
            }
            ...
            case "deleteGroup": {
                String groupName = msg.getData();
                if (!database.isAdminOfGroup(groupName, msg.getUsername())) {
                    sendTo(msg.getConn(), new Message("deleteGroup", msg.getUsername(), msg.getUsername(),
                            "You are not admin of this group"));
                    return;
                }
            ...
            case "getMotivationalPhrases": {
                ...
                for (String phrase : phrases) {
                    sendTo(msg.getConn(),
                            new Message("getMotivationalPhrases", msg.getUsername(), msg.getUsername(), phrase));
                }
            }
                break;
            case "addMotivationalPhrase": {
                ...
                sendTo(msg.getConn(),
                        new Message("addMotivationalPhrase", msg.getUsername(), msg.getUsername(), "success"));
            }
